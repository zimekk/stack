...
export default class Chat extends Component {
  ...
  componentDidMount() {
    ...
    `)
      .then(data => console.log(data) || data.messages.map(({ text }) => text))
      .then(messages => setTimeout(() => this.setState({ messages }), 800));
  }
  onSend = () =>
    query(
      gql`
      ...
      `,
      { text: `Message #${this.state.counter}` }
    )
      .then(
        data => console.log(data) || data.Message.add.map(({ text }) => text)
      )
      .then(messages =>
        setTimeout(
          () => this.setState({ counter: this.state.counter + 1, messages }),
          800
        )
      );
  onClear = () =>
    ...
    `)
      ...
      .then(() => setTimeout(() => this.setState({ messages: [] }), 200));
